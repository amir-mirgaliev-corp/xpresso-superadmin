<template>
	<section class="transport-marks__content">
		<div class="transport-marks__container max-w-4xl rounded-[12px] border border-[#DCDCDC] p-6">
			<h1 class="text-2xl font-bold text-gray-800 mb-6">Марка автомобиля</h1>
    ...
    <DangerModal
        ...
        @confirm="deleteMark"
        @close="deleteModalOpen = false; deleteMarkId = null;"/>
</template>
    ...
    computed: {
        ...
        groupTransportModelsByCategory() {
            ...
            return this.getTransportCategories.reduce((grouped, category) => {
                grouped[category.name] = this.getTransportModels.filter(
