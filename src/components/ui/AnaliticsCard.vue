        ...
        <div class="analytics__card-container">
            <div class="analytics__card-content flex justify-between">
                ...
                <div class="analytics__card-container-icon">
                    <span class="analytics__card-icon border-[2px] rounded-[10px] shadow-drop w-12 h-12 flex justify-center items-center">
                        <i class="fi text-xl" :class="analytic.icon"></i>
                    </span>
                </div>
            </div>

            <div v-if="analytic.profit > 0 || analytic.los > 0" class="analytics-line w-full h-[1px] m-[20px_0] bg-gray-300"></div>
            ...
            <div class="analytics__card-statistic flex justify-between" v-if="analytic.statistic">
                <div v-if="analytic.profit > 0" class="analytics__card-statistic-container flex items-center gap-[2px]">
                    <div class="overflow-hidden">
                        <i class="fi fi-rr-arrow-small-up text-2xl text-[#17B26A] animated-arrow arrow-up" :ref="setArrowRef"></i>
                    </div>
                    <h2 class="analytics__card-statistics-sum text-lg text-[#141414] font-bold"> +{{ formatAmount(analytic.profit) }} {{ analytic.unit }}</h2>
                </div>

                <div v-if="analytic.los > 0" class="analytics__card-statistic-container flex items-center gap-[2px]">
                    <div class="overflow-hidden">
                        <i class="fi fi-rr-arrow-small-down text-2xl text-[#F2271C] animated-arrow arrow-down" :ref="setArrowRef"></i>
                    </div>
                    <h2 class="analytics__card-statistics-sum text-lg text-[#141414] font-bold"> -{{ formatAmount(analytic.los) }} {{ analytic.unit }}</h2>
                </div>
            </div>
